...
const roles = [
  { title: 'Guardian', icon: <Shield size={24} />, desc: 'Vetted women 18+ who complete a 40-hour course and get paid to support others.', color: '#BF6EEE' },
  { title: 'Community Manager', icon: <Users size={24} />, desc: 'Lead your local community — bring women together and champion safety.', color: '#9A4FD8' },
  { title: 'Campus Ambassador', icon: <Star size={24} />, desc: 'Bring Kinnav to your university and make campus safer for everyone.', color: '#D4A5F5' },
  { title: 'Reward Partner', icon: <Heart size={24} />, desc: 'Wellness brands offering yoga, counseling, and coaching to Kinnav members.', color: '#7B2FB8' },
  { title: 'NGO Partner', icon: <Globe size={24} />, desc: 'Advocacy orgs creating empowerment content and earning per engagement.', color: '#FD62D8' },
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }} className="two-col">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <img src="/images/guardian-network.jpg" alt="Guardian network" style={{ width: '100%', borderRadius: 24, boxShadow: '0 20px 60px rgba(191,110,238,0.2)' }} />
            </motion.div>
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
            {team.map((m, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                style={{ background: '#fff', borderRadius: 20, padding: '2rem', boxShadow: '0 4px 20px rgba(191,110,238,0.08)', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.25rem' }}>
            {values.map((v, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}
                style={{ background: '#FAF5FF', borderRadius: 16, padding: '1.5rem', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
      ...
      <section style={{ padding: '5rem 1.5rem', background: '#FAF5FF' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
            {roles.map((r, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}
                style={{ background: '#fff', borderRadius: 18, padding: '1.5rem', display: 'flex', gap: 14, alignItems: 'flex-start', boxShadow: '0 4px 20px rgba(191,110,238,0.07)' }}>
        ...
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            ...
            <motion.div variants={fadeUp} style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link to="/contact" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '13px 30px', borderRadius: 50, background: 'linear-gradient(135deg, #D4A5F5, #BF6EEE)', color: '#fff', fontWeight: 700, fontSize: 15, textDecoration: 'none' }}>
