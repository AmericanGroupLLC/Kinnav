...
const features = [
  { icon: <Map size={26} />, title: 'Guardian Map', desc: 'See vetted guardians near you on a live map. Real people, always nearby.', color: '#9B59D0' },
  { icon: <Phone size={26} />, title: 'Safe Call', desc: 'One tap — voice, video, or text with a guardian. No time limit.', color: '#7B3FBE' },
  { icon: <Shield size={26} />, title: 'Emergency', desc: 'Instantly alert emergency services and notify your safety contacts.', color: '#E53935' },
  { icon: <BookOpen size={26} />, title: 'Empowerment', desc: 'Self-defense, safety planning, wellness modules — all in the app.', color: '#6A1B9A' },
  { icon: <Users size={26} />, title: 'Be a Guardian', desc: 'Complete a 40-hour course, get verified, and earn while helping others.', color: '#B57BE0' },
  { icon: <Gift size={26} />, title: 'Rewards', desc: 'Earn wellness rewards — yoga, counseling, career coaching, and more.', color: '#9B59D0' },
]
  ...
  return (
    <div>
      ...
      <section style={{
        ...
        <div style={{
          position: 'absolute', inset: 0,
          background: 'linear-gradient(135deg, rgba(26,5,51,0.93) 0%, rgba(61,18,120,0.87) 45%, rgba(106,27,154,0.75) 70%, rgba(155,89,208,0.55) 100%)',
        }} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '3rem', alignItems: 'center' }} className="hero-grid">
            <motion.div initial="hidden" animate="visible" variants={stagger}>
              <motion.div variants={fadeUp} style={{
                ...
                background: 'rgba(181,123,224,0.2)', border: '1px solid rgba(181,123,224,0.4)',
                borderRadius: 50, padding: '7px 16px', marginBottom: 24,
                color: '#D8C4F0', fontSize: 13, fontWeight: 600,
              }}>
                <Shield size={15} style={{ color: '#B57BE0' }} />
      ...
      {/* ── STATS BAR ── */}
      <section style={{ background: 'linear-gradient(135deg, #6A1B9A, #9B59D0)', padding: '2.5rem 1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
            {steps.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                style={{ background: '#F9F5FF', borderRadius: 20, padding: '2rem 1.5rem', textAlign: 'center' }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'linear-gradient(135deg, #B57BE0, #9B59D0)', color: '#fff', fontWeight: 900, fontSize: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>{s.num}</div>
      ...
      <section style={{ padding: '5rem 1.5rem', background: '#F9F5FF', overflow: 'hidden' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            {[
              ...
              <motion.div key={src}
                ...
                <div style={{
                  background: '#1a0533', borderRadius: 28, padding: 6,
                  boxShadow: '0 20px 50px rgba(155,89,208,0.25)',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
            {features.map((f, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}
                style={{ background: '#F9F5FF', borderRadius: 18, padding: '1.5rem', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                    ...
                    <tr key={row.name} style={{ background: row.highlight ? 'rgba(155,89,208,0.2)' : 'transparent', borderRadius: 12 }}>
                      ...
                      {[row.s, row.e, row.r].map((v, i) => (
                        <td key={i} style={{ padding: '14px 16px', textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                          {v ? <span style={{ color: '#B57BE0', fontSize: 20 }}>✓</span> : <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: 18 }}>—</span>}
                        </td>
                      ))}
                    </tr>
                  ))}
